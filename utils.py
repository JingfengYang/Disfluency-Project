...
def preProcess(sents):
    ...
    for sent in sents:
        ...
        sent2 = []
        for t in sent:
            if not t[0].endswith('-') and not t[0] == 'SILENCE' and \
                    not t[0] == 'TRACE' and not t[1] == 'None':
                sent1.append(t)
        if len(sent1) <= 0:
            continue
        tag = 0
...
def batchIter(data, batch_size, tag_pad=0, train=True):
    ...
    for i in range(nbatch):
        yield padding(allSents[i * batch_size:(i + 1) * batch_size]), \
            padding(allTags[i * batch_size:(i + 1) * batch_size], pad=tag_pad)
    if len(allSents) > nbatch * batch_size:
        yield padding(allSents[nbatch * batch_size:]), \
            padding(allTags[nbatch * batch_size:], pad=tag_pad)
