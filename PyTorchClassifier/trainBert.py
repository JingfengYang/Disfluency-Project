from utilsBert import build_vocab,batchIter2,isEdit,idData2
...
class Encoder(nn.Module):
    ...
    def forward(self,input,lengths, train=True,hidden=None):
        ...
        encoded_layers, CLS_encoding = self.bert(input)
...
def run_epoch(data_iter, model, loss_compute,train=True,id2label=None):
    ...
    editPredTotal = 0
    editCorrectTotal = 0
    for i, (sent_batch, tag) in enumerate(data_iter):##
        #print(tag_batch[0])
...
for i, label in enumerate(id2label):
    label2id[label] = i
tokenizer = BertTokenizer.from_pretrained('bert-large-uncased-whole-word-masking', do_lower_case=False)
trainData = idData2(tokenizer, 'train.txt', label2id)
valData = idData2(tokenizer, 'val.txt', label2id)
testData = idData2(tokenizer, 'test.txt', label2id)
