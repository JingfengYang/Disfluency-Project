from utilsBertWrite import build_vocab,batchIter,idData,isEdit,writeSents
...
def run(epoch,model,batch_size,trainData,valData,testData,id2label,w_padding):
    ...
    for i in range(epoch):
        ...
        print('Evaluation_train: epoch: %d' % (i))
        loss, f, trainTag = run_epoch(batchIter(trainData, batch_size, w_tag_pad=w_padding, t_tag_pad=len(id2label)), model,
                                    SimpleLossCompute(criterion, optimizer, scheduler), train=False, id2label=id2label)
        print('Loss:', loss)
        if len(trainResult) == 0 or f > max(trainResult):
            trainLabels = trainTag
        trainResult.append(f)
        ...
        if len(testResult)==0 or f >max(testResult):
            testLabels=testTag
        testResult.append(f)
    trainBest = max(trainResult)
    print('TrainBest epoch:', [i for i, j in enumerate(trainResult) if j == trainBest])
    writeResults('predictTrain', trainLabels, trainData[3])
    valBest=max(trainResult)
    print('ValBest epoch:', [i for i, j in enumerate(valResult) if j == valBest])
    writeResults('predictVal',valLabels,valData[3])
    testBest = max(testResult)
    print('TestBest epoch:', [i for i, j in enumerate(testResult) if j == testBest])
    writeResults('predictTest', testLabels,testData[3])
