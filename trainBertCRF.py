from utilsBertCRF import build_vocab,batchIter,idData,isEdit
...
class SimpleLossCompute:
    ...
    def __call__(self, x, y, mask,norm,train=True):
        pad = x.size(-1)
        y.masked_fill_(y == pad,0.0)
        loss = -self.criterion(x,y,mask=mask, reduction='sum') / norm
...
def run_epoch(data_iter, model, loss_compute,train=True,id2label=None):
    ...
    for i, (sent_batch,tag_batch,head_batch,tag_mask_batch) in enumerate(data_iter):##
        ...
        if i %  PRINT_EVERY== 0:
            ...
            tokens = 0
        if not train:
            results=loss_compute.criterion.decode(out,mask=masks)
            pad=out.size(-1)
            y = tag_batch[0].contiguous().detach().tolist()
            length=tag_mask_batch[1].contiguous().detach().tolist()
            #print('results:',results,y)
            for predS,goldS,lenS,m in zip(results,y,length,masks):
                assert(m.size(0)>=lenS)
                #if not [1]*lenS+[0]*(m.size(0)-lenS) == m.contiguous().detach().tolist():
                    #print(len([1]*lenS+[0]*(m.size(0)-lenS)),len(m.contiguous().detach().tolist()),[1]*lenS+[0]*(m.size(0)-lenS) , m.contiguous().detach().tolist())
                assert([1]*lenS+[0]*(m.size(0)-lenS) == m.contiguous().detach().tolist())
                #if not(lenS==len(predS)):
                    #print(lenS,len(predS))
                assert(lenS==len(predS))
...
def run(epoch,model,batch_size,trainData,valData,testData,id2label,w_padding,criterion):
    valResult=[]
    testResult=[]
...
run(EPOCH,encoder,BATCH_SIZE,trainData,valData,testData,id2label,tokenizer._convert_token_to_id('[PAD]'),criterion)