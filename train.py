...
class Encoder(nn.Module):
    ...
    def forward(self,input,lengths, train=True,hidden=None):
        input=self.relu1(self.embeds2input(self.word_embeds(input)))
        if train:
                ...
                if not gold==pad:
                    if isEdit(pred,id2label) and isEdit(gold,id2label):
                        ...
                        if isEdit(gold,id2label):
                            editTrueTotal +=1
    if not train:
        print(editTrueTotal, editPredTotal, editCorrectTotal)
        ...
        print("Edit word precision: %f recall: %f fscore: %f" % (p, r, 2 * p * r / (p + r)))
...
def run(epoch,model,batch_size,trainData,valData,testData,id2label):
    ...
    for epoch in range(epoch):
        ...
        print(run_epoch(batchIter(valData,batch_size,tag_pad=len(id2label)), model,
