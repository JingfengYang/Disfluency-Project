...
class Encoder(nn.Module):
    def __init__(self, word_size, word_dim, input_dim, hidden_dim,
                 nLayers, labelSize, dropout_p):
...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        true_dist.scatter_(1, target.masked_fill(
            target == self.padding_idx, 0).unsqueeze(1), self.confidence)
...
def run(epoch, model, batch_size, trainData, valData, testData, id2label):
    ...
    criterion = LabelSmoothing(
        size=len(id2label), padding_idx=len(id2label), smoothing=0.0)
    ...
    for i in range(epoch):
        model.train()
        run_epoch(batchIter(trainData, batch_size, tag_pad=len(id2label)),
                  model, SimpleLossCompute(criterion, model_opt), train=True)
        model.eval()
        print('Evaluation_val: epoch: %d' % (i))
        loss, f = run_epoch(
            batchIter(valData, batch_size, tag_pad=len(id2label)),
            model, SimpleLossCompute(criterion, model_opt),
            train=False, id2label=id2label)
        print('Loss:', loss)
        valResult.append(f)
        print('Evaluation_test: epoch: %d' % (i))
        loss, f = run_epoch(
            batchIter(testData, batch_size, tag_pad=len(id2label)),
            model, SimpleLossCompute(criterion, model_opt),
            train=False, id2label=id2label)
        print('Loss:', loss)
        testResult.append(f)
...
print(id2label)
encoder = Encoder(len(vocDic), WORD_EMBEDDING_DIM, INPUT_DIM,
                  HIDDEN_DIM, ENCODER_LAYER, len(id2label),
                  DROUPOUT_RATE).to(device)
